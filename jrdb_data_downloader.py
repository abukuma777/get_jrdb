...
# def download_and_organize_Paci():
#     """
#     JRDBのPaciデータをダウンロードし、各ファイルタイプに基づいて整理する関数。
...
#     5. ダウンロードしたデータを解凍
#     6. 解凍した各テキストファイルを対応する年度・ファイルタイプのフォルダに整理
...
#     """
#     # JRDBの基本URL
#     JRDB_BASE_URL = "http://www.jrdb.com/member/data/Paci/"
...
#                 # 解凍（lhaコマンドが必要、出力を無視）
#                 os.system(f"lha -xw=tmp/ {tmp_file_path} > /dev/null 2>&1")

#                 # 解凍したテキストファイルを整理
#                 for txt_file in os.listdir("tmp/"):
#                     if txt_file.endswith(".txt"):
#                         file_type = txt_file[:3]

#                         # ファイルの作成日時を取得
#                         created_time = os.path.getctime(f"tmp/{txt_file}")
#                         created_date = datetime.datetime.fromtimestamp(created_time)
#                         year = created_date.year  # 年度を取得

#                         target_folder = f"/app/data/jrdb_txt/{file_type}/{year}/"
...
def download_and_organize_jrdb_data(File_type):
    """
                ...
                for txt_file in os.listdir("tmp/"):
                    if txt_file.endswith(".txt"):
                        ...
                        if match:
                            ...
                            # 年度フォルダを作成
                            year_folder = f"/app/data/jrdb_txt/{File_type}/{year}/"
                            if not os.path.exists(year_folder):
                                os.makedirs(year_folder)

                            target_folder = f"/app/data/jrdb_txt/{match.group(1)}/{year}/"
                            if not os.path.exists(target_folder):
                                os.makedirs(target_folder)
                            shutil.move(f"tmp/{txt_file}", f"{target_folder}/{txt_file}")
